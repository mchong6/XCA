...
dofile './provider_texture_no_norm.lua'
...
opt = lapp[[
   ...
   --model                    (default vgg_bn_drop_texture_hot)     model name
...
test_Y = torch.cat({torch.repeatTensor(label0, 1000, 1),torch.repeatTensor(label1, 1000, 1)}, 1)  
...
provider = torch.load 'provider_texture_no_norm.t7'

image.save("train.png", provider.trainData.data[1])
image.save("train2.png", provider.testData.data[1])
provider.testData.data = torch.cat({provider.trainData.data[{{4001,5000}}]:float(),provider.trainData.data[{{9001,10000}}]:float()},1)
