...
y:fill(2)
print(x)
x will be changed also because y is a copy of x pointer
]]
require 'xlua'
require 'optim'
require 'nn'
...
--change the dofile to the dataset creation file.
...
--create label for the 2 classes
...
--create the correct output for training set. Here we use 4000 class A and 4000 class B
train_Y = torch.cat(torch.repeatTensor(label0, 4000, 1), torch.repeatTensor(label1, 4000, 1), 1)


--create label for test set. Includes exclusive A, exclusive B and common
...
--create correct output for testing set
...
--In my dataset i have 5000 class A and 5000 class B in testData. Here I split to 4000 to train and 1000 to test
...
--labels are the correct output for all the training set.
...
--sanity check. Print out the images to check if dataset is correct
...
--SpatialClassNLLCriterion here is MSE
...
function train()
  ...
  print(c.blue '==>'.." online epoch # " .. epoch .. ' [batchSize = ' .. opt.batchSize .. ']')
	...
	--randperm is random permutation. Randomly split batchsize	
  ...
  for t,v in ipairs(indices) do
	...
	--outputs is the last layer output. Used to calculate MSE
      local f = criterion:forward(outputs, targets)
	--f is the MSE for current batch
      error = error + f
	...
	--initialize MSEPLOTa to plot MSE error
  if MSEPLOT == nil then
...
function test()
  ...
  model:evaluate()
	--pointer to layer before label for printing output space
  ...
  local bs = 100
	--outputsum is the output of all the testdata through the network
  local outputSum = torch.Tensor(2300,labelLength):cuda()
  for i=1,provider.testData.data:size(1),bs do
	--forward propagate testdata batch
    local outputs = model:forward(provider.testData.data:narrow(1,i,bs))
	--for every 30 epoch and for i <=2000 such that we are testing testA and testB, save the point mapping
	if epoch % 30 == 0 and i <= 2000 then
		if outs_XA == nil then 
			--clone it so we dont affect the network
 ...
 --confusion:updateValids()
	--pass outputs into accuracy which calculates accuracy of the classification
