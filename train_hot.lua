...
opt = lapp[[
   ...
   --model                    (default vgg_2class_bad)     model name
...
function train()
  ...
  for t,v in ipairs(indices) do
    ...
    provider.trainData.labels = torch.cat(torch.repeatTensor(label0, 5000, 1), torch.repeatTensor(label1, 5000, 1), 1)
    provider.testData.labels = torch.cat(torch.repeatTensor(label0, 500, 1), torch.repeatTensor(label1, 500, 1), 1)  
  ...
  local lastlayer = model.modules[model:size(1)].modules[model.modules[model:size(1)]:size(1)].modules[model.modules[model:size(1)].modules[model.modules[model:size(1)]:size(1)]:size(1)-1]
    ...
    --print(lastlayer.gradWeight)
      ...
      local df_do = criterion:backward(outputs, targets, switch)
...
function test()
  ...
  local bs = 125
  local outputSum = torch.Tensor(1000,2):cuda()
