...
opt = lapp[[
   ...
   --model                    (default vgg_bn_drop2)     model name
...
function train()
  ...
  local lastlayer = model.modules[model:size(1)].modules[model.modules[model:size(1)]:size(1)].modules[model.modules[model:size(1)].modules[model.modules[model:size(1)]:size(1)]:size(1)-1]
	...
	switch = torch.random(0,1)
      ...
      model:backward(inputs, df_do)
        --print((grad_Decor * W_Decor_mult)[1][{{1,5}}])
        --print(lastlayer.gradWeight[1][{{1,5}}])
	lastlayer.gradWeight:add(grad_Decor * W_Decor_mult)
      --print(lastlayer.gradWeight)
  ...
  if MSEPLOT == nil then
      MSEPLOT = torch.Tensor(1):fill(error/provider.trainData.data:size(1))
      WEIGHTPLOT = torch.Tensor(1):fill(error_decor/provider.trainData.data:size(1))
  else
      MSEPLOT = torch.cat(MSEPLOT, torch.Tensor(1):fill(error/provider.trainData.data:size(1)), 1)
      WEIGHTPLOT = torch.cat(WEIGHTPLOT, torch.Tensor(1):fill(error_decor/provider.trainData.data:size(1)), 1)
  end
...
function test()
  ...
  local bs = 125
  local outputSum = torch.Tensor(1000,8):cuda()
